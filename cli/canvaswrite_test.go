...
func TestCanvasWriteRefusalsLeaveTheFileAlone(t *testing.T) {
	...
	cases := []struct {
		...
		{"read word with an empty write flag", []string{"show", "--at="}, "usage", "takes no --at"},
		{"empty flag the word does not take", []string{"place", auth, "--at", "1,2", "--title="}, "usage", "does not take --title"},
		{"empty flag the word needs", []string{"inbox", "--at="}, "usage", "X,Y"},
