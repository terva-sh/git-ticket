...
The behaviour it describes ships in git-ticket as `import --same-owner`. Until a
release carries it, the procedure below is accurate against `main` and not
against any tag, so do not paste it into ledger before the version note at the
bottom is settled.
...
`--same-owner` is on `main` in `terva-sh/git-ticket` and is not in `v0.17.1`.
Confirm the version ledger installs actually has it:
