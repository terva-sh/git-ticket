...
// nothing else, which was fine while CSI was the only escape emitted
// into a measured line. OSC 8 hyperlinks changed that: an OSC sequence
// carries a URL that occupies no cells, so a scanner that does not know
// it counts a hundred-odd bytes of link target toward the visible
// column and shreds the layout around it. These two helpers are the
// single definition of "an escape starts here, and it is this long",
// so the CSI and OSC rules cannot drift between callers.
...
// to the end of s, matching what a terminal would do with the bytes.
