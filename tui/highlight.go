...
//
// The lift drops LanguageFromPath and its extension tables: the detail
// view highlights fenced blocks, which carry their language hint on
// the fence, and no caller here has a file path to map.
...
// returns the result split into lines, ready for a line-based
...
// goroutines.
//
// Results are memoised by (style, lang, src) so repeated calls from the
// view builder (which runs on every rebuild) don't re-tokenise. The
// cache is bounded and evicts the oldest half past its cap.
...
// order). That's good enough since a ticket's sections don't change
// between rebuilds, so cache hits are frequent and evictions rare.
...
// (notably punctuation/error-ish spans), which looks like random black
// blocks against any other background. Foreground color is enough for code.
