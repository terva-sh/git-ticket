// Lifted from terva's packages/tui (MIT), see NOTICE at the
// repository root:
//   Copyright (c) 2026 Drew Short (Terva, a hard fork of zot)
//   Copyright (c) 2026 Patric Eckhart
//
// The lift drops FlushLeftSentinel, which terva keeps only for callers
// that predate its current fence rendering; no caller here ever
// stripped it.
...
//
// width bounds table layout. Pass 0 to use a reasonable fallback.
func RenderMarkdown(src string, th Theme, width int) string {
	...
	// horizontal rules. Syntax highlighting alone is enough to signal
	// "this is code"; unambiguous because prose doesn't use the accent
	// palette.
	flushFence := func() {
		if fenceBuf.Len() == 0 {
			return
	...
	// Handle truncated input: the opening ``` arrived but the closing
	// one hasn't. Emit the buffered content so the partial fence still
	// reads cleanly.
	if inFence {
		flushFence()
	}
...
func renderTableRow(row []string, widths []int, aligns []tableAlign, th Theme, header bool) []string {
	...
	for c := range widths {
		...
		// keep-style: a coloured span (code/bold) wider than the column
		// wraps without the tail dropping to the default colour.
